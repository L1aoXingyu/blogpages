...
2. 使用 MinHash 和 Jaccard similarity 进行近似去重，首先通过 Jaccard 阈值 0.85 构造 duplicate cluster，然后对于每个 cluster，通过 Jaccard 阈值 1 进行精确去重；
3. 过滤掉最大长度大于 1000 或者平均长度大于 100 或者字母字符占比小于 0.25 的文件；
4. 去掉自动生成的文件，比如包含 "auto-generated" 字段，或者前 5 行相似；
5.
